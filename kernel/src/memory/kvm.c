...
void init_page(void) {
	CR0 cr0;
	CR3 cr3;
	PDE *pdir = (PDE *)va_to_pa(kpdir);//页目录的物理地址基址
	PTE *ptable = (PTE *)va_to_pa(kptable);//页表的物理地址基址
	...
	/* fill PDEs and PTEs */
	pframe_idx = 0;
	for (pdir_idx = 0; pdir_idx < PHY_MEM / PT_SIZE; pdir_idx ++) {//物理内存128MB,32个表
		pdir[pdir_idx].val = make_pde(ptable);//第pdir_idx个页目录项指向
